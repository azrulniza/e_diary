...
class UserLeavesController extends AppController
{
    ...
    public function index()
    {
        ...
        if ($userRoles->hasRole(['Master Admin'])) {
            ...
            if(!empty($organizationSelected) AND !empty($staffSelected)){
                $sql_supervisor = "SELECT user_leaves.*, leave_status.`name` AS leave_status_name, leave_types.`name`AS leave_type_name,users.id AS user_id, users.name AS user_name, organizations.id AS organization_id, organizations.`name` AS organizations_name FROM user_leaves 
                ...
                WHERE organizations.id=$organizationSelected AND users.id=staffSelected";
                $stmt_sql_supervisor = $conn->execute($sql_supervisor);
                $get_supervisor = $stmt_sql_supervisor->fetchAll('assoc');
                $userLeaves = $this->paginate($get_supervisor);
            }else if(empty($organizationSelected) AND !empty($staffSelected)){
                ...
                JOIN `user_organizations` ON `user_organizations`.`user_id`= `users`.`id` 
                JOIN organizations ON organizations.id = user_organizations.`organization_id`
                ORDER BY user_leaves.cdate desc";
                $stmt_sql_supervisor = $conn->execute($sql_supervisor);
                $userLeaves = $stmt_sql_supervisor->fetchAll('assoc');
                //$userLeaves = $this->paginate($get_supervisor);
            }
        ...
        $this->set(compact('userLeaves', 'list_organization','list_user','staffSelected','organizationSelected','userRoles'));
    }
    ...
    public function apply()
    {
        ...
        $this->set(compact('userLeave', 'list_user', 'leaveStatuses', 'leaveTypes','userRoles', 'list_organization','staffSelected', 'last_id'));
    }
