...
<?php foreach ($resellers as $reseller): ?>
                            ...
                            <tbody class="ui-sortable">
                                ...
                                <?php foreach ($reseller->users as $key => $user): ?>
                                    <tr id="<?= $user->id; ?>" class="<?= ( ++$count % 2 ? 'odd' : 'even') ?>">
                                        ...
                                        <td><?= h($user->name) ?></td>
                                        <td><?= h($user->email) ?></td>
										<td><?php if($user->status=='1'){ echo 'Active';}
											else if($user->status=='0'){echo 'Disabled';}
											else if($user->status=='2'){echo 'Pending activation';}
											else if($user->status=='3'){echo 'Pending update profile';}
											else if($user->status=='4'){echo 'Password reset requested';}  ?></td>                                         
    ...
    <?php foreach ($reseller->clients as $client): ?>
        <h3><?php echo $client->name ?> <small>(<?php echo __('Client') ?>)</small></h3>

        <?php //debug($client->users) ?>
                    ...
                    <div class="box-body">
                        <div class="users index dataTable_wrapper table-responsive">
                            <table id="dataTables-users" class="dataTable table table-striped table-bordered">
                                ...
                                <tbody class="ui-sortable">
                                    <?php $count = 0; $no=1;?>
                                    <?php foreach ($client->users as $key => $user): ?>
                                        <tr id="<?= $user->id; ?>" class="<?= ( ++$count % 2 ? 'odd' : 'even') ?>">
                                            <td><?= $no ?></td>
                                            <td><?= h($user->name) ?></td>
                                            <td><?= h($user->email) ?></td>
    ...
    <!--<?php foreach ($reseller->downline_resellers as $reseller): ?>
        ...
        <h2><?php echo $reseller->company_name ?> <small>(<?php echo __('Downline Reseller') ?>)</small></h2>
